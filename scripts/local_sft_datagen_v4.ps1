...
Write-Log "START run_id=$RunId workers=$workers model=$($env:TEACHER_MODEL) log=$LogFile"
...
"@
    ...
    & $Py (Join-Path $Data1 "scripts\run_pipeline.py") `
        --mode live `
        --config $CanaryCfg `
        --work-dir $CanaryWork `
        --track sft `
        ...
        --workers $workers `
        --budget-usd $canaryBudget `
        --no-resume *>> $LogFile
    if ($LASTEXITCODE -ne 0) { throw "Canary run_pipeline failed exit=$LASTEXITCODE" }
    ...
    & $Py (Join-Path $Data1 "scripts\run_pipeline.py") `
        ...
        --budget-usd $budget `
        --no-resume *>> $LogFile
    if ($LASTEXITCODE -ne 0) { throw "Full SFT run_pipeline failed exit=$LASTEXITCODE" }
    ...
    & $Py (Join-Path $Data1 "scripts\select_sft_v4_release.py") `
        --candidates $cand `
        --out $selected `
        --allow-missing-decontam *>> $LogFile
    if ($LASTEXITCODE -ne 0) { throw "select_sft_v4_release failed exit=$LASTEXITCODE" }

    $dest = Join-Path $RepoRoot "data\arabic_reasoning_coldstart_v4.jsonl"
    ...
    # Stable pointer for the monitor loop
