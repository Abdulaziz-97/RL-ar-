...
Plug-and-play CLI.
    ...
    python -m rlvr_sota sft   --config configs/qwen_4b_qlora.yaml
    python -m rlvr_sota train --config configs/qwen_4b_smoke_v11.yaml --sft-checkpoint ./runs/sft_v1
    python -m rlvr_sota eval  --config configs/qwen_4b_qlora.yaml --data data/... --checkpoint outputs/
"""
...
from rlvr_sota.trainer import build_trainer, build_sft_trainer
...
def _apply_overrides(config: SOTAConfig, args: argparse.Namespace) -> SOTAConfig:
    overrides = [
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "audit-coldstart":
        from rlvr_sota.audit_coldstart import main as audit_main
    ...
    config = SOTAConfig.from_yaml(args.config)
